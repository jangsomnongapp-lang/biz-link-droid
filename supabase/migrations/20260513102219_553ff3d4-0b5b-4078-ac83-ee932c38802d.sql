...
-- Run a lottery draw: pick a random active ticket and create a prize claim with 48h expiry.
...
BEGIN
  ...
  -- Find BuildHub Rewards super-user (the official identity); fallback to first admin
  ...
  -- Open / find DM thread between Rewards and winner
...
-- Expire claims older than 48h that were never accepted
...
-- Auto-create scheduled draws (daily / weekly / monthly) so the cron can pick them up
...
-- Cron: every hour expire overdue claims; every day at 21:00 UTC ensure scheduled draws
CREATE EXTENSION IF NOT EXISTS pg_cron;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'rewards-expire-claims') THEN
    PERFORM cron.schedule('rewards-expire-claims', '0 * * * *',
      $cron$ SELECT public.expire_overdue_claims(); $cron$);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'rewards-ensure-draws') THEN
    PERFORM cron.schedule('rewards-ensure-draws', '0 21 * * *',
      $cron$ SELECT public.ensure_scheduled_draws(); $cron$);
  END IF;
END $$;

-- Seed today's scheduled draws
SELECT public.ensure_scheduled_draws();
