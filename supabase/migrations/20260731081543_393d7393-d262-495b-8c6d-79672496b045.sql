...
AS $$
BEGIN
  -- Trusted contexts: service role, table owner, or SECURITY DEFINER functions
  -- owned by postgres. Direct API writes run as `authenticated` / `anon`.
...
WHERE email LIKE 'ptest%@project001.local';