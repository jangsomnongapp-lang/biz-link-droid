  ...
  IF is_avail THEN
    ...
    IF has_daily_ticket THEN
      SELECT ticket_number INTO assigned_num FROM public.lottery_tickets
        WHERE user_id = uid AND ticket_type = 'daily' AND draw_period_start = today
        LIMIT 1;
      RETURN jsonb_build_object('already', was_already_today, 'status', _status, 'ticket_number', assigned_num);
    END IF;
    ...
    IF NOT FOUND THEN
      ...
      new_streak := 1;
    ELSE
      IF prev.last_check_date = today THEN
        new_streak := GREATEST(prev.current_streak, 1);
